...
function editDiv(){
    ...
    getJSON("https://www.khanacademy.org/api/internal/scratchpads/top?casing=camel&sort=3&page="+page+"&limit=6&format=pretty&topic_id=xffde7c31&cursor="+cursor, function(data){
        cursor = data.cursor
...
getJSON("https://www.khanacademy.org/api/internal/scratchpads/top?casing=camel&sort=3&page="+page+"&limit=6&format=pretty&topic_id=xffde7c31", function(data){
    cursor = data.cursor
    for(var i = 0; i < data.scratchpads.length; i++){
        var info = data.scratchpads[i]
        var url = info.url
